...
class imageSaver(object):
    ...
    def start(self):
        ...
        # Get image UUID
        self.image_uuid = self.imaging_api.computerCreateImageDirectory(
            self.manager.mac)
        ...
        error_code = subprocess.call(
            'yes 2>/dev/null|/usr/sbin/ocs-sr %s savedisk %s %s 2>&1 1>/dev/null | tee /var/log/davos_saver.log' %
            (self.manager.clonezilla_params['clonezilla_saver_params'], self.image_uuid, self.device), shell=True)

        image_dir = os.path.join('/home/partimag/', self.image_uuid) + '/'

        if error_code != 0:
            self.logger.warning(
                'An error was encountered while creating image, check davos_saver.log for more details.')
            saver_log_path = os.path.join(image_dir, 'davos_saver.log')
            open(
                saver_log_path,
                'w').write(
                open(
                    '/var/log/davos_saver.log',
                    'r').read())
            time.sleep(15)
        ...
        info['title'] = 'Image of %s at %s' % (
            self.manager.hostname, current_ts)
        info['description'] = ''
        info['size'] = sum(
            os.path.getsize(
                image_dir +
