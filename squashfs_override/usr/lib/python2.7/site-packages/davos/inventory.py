...
class Inventory(object):
    ...
    def __init__(self, manager):
        ...
        o, e, ec = manager.runInShell('ip route get ' + manager.tftp_ip)
        if ec != 0:
            manager.logger.error("Can't guess network interface")
            manager.logger.error("%s\n%s", o, e)
            return
        self.interface = o.splitlines()[0].split(' ')[2]
        self.macaddress = netifaces.ifaddresses(self.interface)[netifaces.AF_LINK][0]['addr']
        self.ipaddress = netifaces.ifaddresses(self.interface)[netifaces.AF_INET][0]['addr']
        self.netmask = netifaces.ifaddresses(self.interface)[netifaces.AF_INET][0]['netmask']
